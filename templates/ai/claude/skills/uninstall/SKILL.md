...
- **원본 복원** 선택 항목:
  ```bash
  bash templates/ai/claude/skills/uninstall/scripts/unlink.sh <도구> --restore
  ```

- **현재 설정 유지** 선택 항목:
  ```bash
  bash templates/ai/claude/skills/uninstall/scripts/unlink.sh <도구> --keep
  ```

- **심링크만 제거** (백업 없음):
  ```bash
  bash templates/ai/claude/skills/uninstall/scripts/unlink.sh <도구> --remove-only
  ```
